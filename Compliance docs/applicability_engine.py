"""
NBFC Applicability-Rule Engine — Phase 1 reference implementation.
...
def evaluate_template(tpl: dict, profile: dict) -> list[ObligationResult]:
    ...
    if decision == Decision.APPLICABLE and is_per_state:
        if state_keys_present:
            ...
            states = sorted(set(profile.get("operating_states", [])) & allowed)
        else:
            states = sorted(profile.get("operating_states", []))
        out = []
        for st in states:
...
if __name__ == "__main__":
    ...
    GOLDEN = {
        ...
        # C deliberately leaves several soft flags unanswered -> NEEDS_REVIEW
        "C_incomplete_profile": {
    ...
    for name, prof in GOLDEN.items():
        res = generate_compliance_universe(lib, prof)
        s = res["summary"]
        print(f"\n=== Profile {name} ===")
        print(f"  Applicable: {s['applicable']} | Needs review: {s['needs_review']} | "
              f"N/A: {s['not_applicable']} | Laws: {s['laws_touched']} | "
              f"Provisional: {s['library_provisional']}")
        # show a couple of state-expanded + review examples
        states_ex = [r['template_id'] for r in res['applicable'] if r['state']][:6]
        print("  State-expanded e.g.:", states_ex)
        rev = [(r['template_id'], r['missing_fields']) for r in res['needs_review']][:5]
        print("  Needs-review e.g.:", rev)
