"""
NBFC Document Intelligence — Phase 1 reference (deterministic components).
...
behind a clean interface (classify_and_extract) and stubbed here. The AI layer
SUGGESTS; the human confirms (Maker-Checker is unchanged).
"""
...
def route(confidence: float) -> str:
    if confidence >= HIGH: return "AUTO_SUGGEST"      # pre-checked link, user confirms
    if confidence >= MED:  return "SUGGEST_REVIEW"     # surfaced, user must confirm type+link
...
# ---------------------------------------------------------------------------
# Interface to the AI layer (OCR + LLM) — stubbed here.
...
def classify_and_extract(file_bytes: bytes, mime: str, hint: dict | None = None) -> dict:
    raise NotImplementedError("Wired to OCR + Claude in production; see spec §4–5.")


if __name__ == "__main__":
    from collections import Counter
    lib = json.load(open("/mnt/user-data/outputs/nbfc_obligation_library_seed.json"))
    O = lib["obligation_templates"]

    # --- Coverage test: map every required_evidence string to a canonical type ---
    all_ev = [e for o in O for e in o["required_evidence"]]
    mapped = [(e, map_evidence_to_type(e).value) for e in all_ev]
    dist = Counter(t for _, t in mapped)
    other = [e for e, t in mapped if t == "OTHER"]
    print(f"Evidence strings mapped: {len(all_ev)}")
    print("Type distribution:", dict(dist.most_common()))
    print(f"Unmapped (OTHER): {len(other)}  -> {sorted(set(other))[:10]}")

    # --- Scenario: TDS deposit, March 2026, user uploads a challan ---
    tpl = next(o for o in O if o["template_id"] == "it_tds_deposit")
    ...
    extracted = {  # as the LLM/OCR would return for an ITNS-281 challan
    ...
    print("\n-- Validation: ITNS-281 challan vs TDS-deposit Mar-2026 instance --")
    for c in validate(extracted, instance, tpl, org):
        print(f"  [{c.result.upper():4}] {c.name:16} {c.detail}")

    classification_conf = 0.93
    print(f"\nClassification: PAYMENT_CHALLAN @ {classification_conf} -> route={route(classification_conf)}")

    # --- Completeness for that instance ---
    comp = completeness(tpl, linked_doc_types=["PAYMENT_CHALLAN"])
    print("\n-- Completeness (challan linked, computation not yet) --")
    print(f"  required: {comp['required']}")
    print(f"  covered : {comp['pct']}% | primary_present={comp['primary_present']} "
          f"| eligible_for_completion={comp['eligible_for_completion']}")
    print(f"  missing : {comp['missing']}")

    # --- Duplicate detection ---
    d1 = {"id": "doc1", "sha256": "abc", "entity_id": "e1",
          "ai_extracted": {"form_number": "ITNS-281", "period": "Mar 2026", "reference_number": "00123"}}
    d2_exact = {"id": "doc2", "sha256": "abc"}
    d2_near = {"id": "doc3", "sha256": "xyz", "entity_id": "e1",
               "ai_extracted": {"form_number": "ITNS-281", "period": "Mar 2026", "reference_number": "00123"}}
    print("\n-- Duplicate detection --")
    print("  exact:", dedupe(d2_exact, [d1]))
    print("  near :", dedupe(d2_near, [d1]))
