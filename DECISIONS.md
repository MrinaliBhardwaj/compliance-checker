...
- **`app/content/` is a register of primary instruments, not a scraper.** A human
  downloads the document from the regulator, drops it under `content/mirror/`,
  and runs `python -m app.content.pipeline mirror <id> <file> --by "<name>"`.
  Retrieval is a deliberate, attributable act with a date on it. `rbi.org.in` is
  not reachable from CI or a sandbox, and a regulator's site does not belong in a
  code path — but the deeper reason is that a scraper cannot sign anything.
- **A sign-off binds to a document digest, not to a source name.** Verifying a
  template against "the SBR Master Direction" means nothing once that document is
  amended. `Threshold(status=VERIFIED)` now requires `source_id`, `verified_by`,
  `verified_on` **and** `verified_against` (sha256), enforced in `__post_init__`.
- **That is what makes content a subscription rather than a snapshot.**
  Re-mirroring an amended instrument changes its digest, and every derivation
  signed against the old digest goes STALE and returns to the queue — without
  anyone remembering to check. `test_reviewing_an_amended_source_makes_the_signoff_stale`
  proves the whole cycle end to end.
- **The queue distinguishes blocking from expected.** Unverified is the honest
  state before a reviewer is engaged; stale sign-offs and mirror-integrity
  failures mean something we already claimed is no longer supported. `pipeline
  check` in CI fails only on the latter — failing on merely-unverified would
  train everyone to ignore it.
- **All 5 thresholds are bound to a real source id**, asserted by
  `test_every_threshold_is_bound_to_a_real_source`. A threshold bound to nothing
  can never be detected as stale, so the queue reports orphans separately.
- **10 instruments registered, 0 mirrored** — accurate, and the point: the queue
  opens with "nobody can verify against a document nobody has obtained." First
  real task for the CS is mirroring `rbi_sbr_md_2023` and `rbi_crilc_nbfc`.
- **AI layer deliberately not built.** The Phase 3 plan said "AI layer last, and
  only where a customer asked twice." There are no customers, so there is no
  second ask. Building the vector store now would be the most comfortable way to
  avoid the two things that actually matter: a reviewer and a pilot.
- **Verified metrics:** 197 tests (189 everywhere + 8 Postgres-only), 85% coverage.

## 2026-08-18 — Phase 3a: real DDL baseline, and a false-green RLS test

- **`0001_baseline_schema.py` is explicit DDL now**, autogenerated against a real
  PostgreSQL 16 server and committed. It previously called
  `Base.metadata.create_all()` on the reasoning that migration and ORM "can never
  drift" — a guarantee that only ever held for a *fresh* database. `create_all`
  skips existing tables and never issues an ALTER, so from the second deploy a
  model change would apply to nothing, silently; and autogenerate had no baseline
  to diff against, so every generated revision would have tried to recreate the
  whole schema. Replaced while no customer database exists, the only cheap moment.
- **Proof, not assertion:** `test_baseline_has_no_drift_from_the_models` runs
  `compare_metadata` against a migrated database and requires an empty diff. Plus
  a full upgrade-head/downgrade-base cycle test.
- **`0004` is deliberately idempotent.** The baseline was rebased on models that
  already include `delivery_status`/`delivery_error`, so a fresh database gets
  them from `0001` and `0004` no-ops; a database still stamped at `0003` needs
  them added. Guarding on the live column list serves both chains. This is the
  standard cost of rebaselining and the reason to do it before customers exist.
- **`test_per_tenant_commit_persists_every_org` was broken and partly vacuous.**
  Three of its INSERTs omitted the NOT NULL `payload` column. Two failed outright;
  the third sat inside `pytest.raises(DBAPIError)` and so **passed on a NOT NULL
  violation while never exercising the RLS policy at all** — a false-green test on
  the product's most important control, the tenant write boundary. Fixed the
  inserts and narrowed the assertion to `match="row-level security"` so it can
  only pass on an actual policy denial. This predates the DDL work; it never ran
  here (Postgres-gated) and CI was red on lint before it, so nothing surfaced it.
- **Verified metrics:** **19** tables (the README claimed 27 — wrong), 185 tests
  (177 everywhere + 8 Postgres-only), 86% coverage. All 8 Postgres tests pass
  under a **non-superuser** role, as they must — a superuser bypasses RLS and
  would make every isolation test pass while the boundary was inert.
